...
chr.names <- c("chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chr12", 
               "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19", "chr20", "chr21", "chr22", "chrX", "chrY", "chr17_ctg5_hap1")
...
gene.bm <- getBM(attributes=attributes, values="*", mart=ensembl) %>% mutate(chromosome_name = paste0("chr", chromosome_name))
gene.bm.mod <- read_tsv("./input/chr17_ctg5_hap1_genes.txt") %>% bind_rows(gene.bm) %>% filter(hgnc_id !="")
...
# heterochromatin
gap.heterochromatin <- gap[gap$type %in% c("heterochromatin", "centromere"),]
...
gr.cen.merged <- reduce(gr.gap.hetochromatin)

## preprocess segdup table
#only in chr1-22, X, Y, and chr17_ctg5_hap1
...
Recurrentdel <- read_tsv("./input/cnv_known.freq.GRCh37.txt") 
...
write_tsv(CNVmerged.final1, "./output/NAHR_GRCh37.withgenes.tsv")
